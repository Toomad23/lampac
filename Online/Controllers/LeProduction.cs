...
namespace Online.Controllers
{
    ...
    //   2. GET /{href}                          -> extract iframe src (id="omfg" / generic / select)
    //   3. GET {iframe}                         -> extract `file: [...]` JSON-ish block
    //   4. Regex out [1080p]https://... and render MovieTpl / EpisodeTpl
    //
    // Upstream parity sync 2026-04-23 (lampac-nextgen f307420):
    //   * Serial support: search now scopes contentType=serial|film and aggregates per-season hrefs.
    //     Season picker (SeasonTpl) appears when >1 season was scraped.
    //   * Episodes parsed from `comment`/`file` pairs in fileBlock; EpisodeTpl built per season.
    //   * Iframe matchers extended (id="omfg<digits>", generic <iframe src=https?://>, <select id="selectFilm">).
    //   * fileBlock matcher accepts `embed|url|default_quality` terminator + `"[1080p]http..."` form.
    //   * Quality dedup + priority sort (1080p first, then 2160p, then descending numeric).
    //   * If no [Q]url found, fall back to first https?:// URL labelled 1080p.
    //
    ...
    //   * Stream URLs are HostStreamProxy'd; SsrfGuard.IsAllowedPublicUriBasic is applied
    //     defensively at every scrape boundary (search href, iframe src, individual streams).
    public class LeProduction : BaseOnlineController
    {
                ...
                if (serial == 1)
                {
                    ...
                    if (s == -1)
                    {
                        ...
                        if (distinctSeasons.Count == 1)
                            return BuildEpisodeTpl(title, original_title, episodes, distinctSeasons[0]);
                        ...
                        foreach (int season in distinctSeasons)
        ...
        StreamQualityTpl ParseStreamQuality(string fileBlock)
        {
            ...
            foreach (Match m in Regex.Matches(fileBlock, "\\[([0-9]+p)\\](https?://[^,\\[\"\t\n ]+)", RegexOptions.IgnoreCase))
            {
                ...
                // Why: streams are scraped; reject non-public destinations before proxying.
                ...
                if (!string.IsNullOrWhiteSpace(link) && !string.IsNullOrWhiteSpace(quality))
                    parsed.Add((HostStreamProxy(link), quality));
            }
            ...
            if (!streamQuality.Any())
            {
                ...
                if (!string.IsNullOrWhiteSpace(single) && SsrfGuard.IsAllowedPublicUriBasic(single))
